...
export interface UseProfileReturn {
  ...
  readonly restoreProfile: (
    userId: string
  ) => Promise<ProfileResult<ProfileRow>>;
  readonly permanentlyDeleteProfile: (
    userId: string
...
export function useProfile(): UseProfileReturn {
  ...
  return {
    ...
    getProfileByUsername: (username) =>
      run(() => repo().getProfileByUsername(username)),
    ...
    updateProfile: (userId, payload) =>
      run(() => repo().updateProfile(userId, payload)),
    updateAvatar: (userId, avatarUrl) =>
      run(() => repo().updateAvatar(userId, avatarUrl)),
    updateBio: (userId, bio) => run(() => repo().updateBio(userId, bio)),
    updatePreferences: (userId, payload) =>
      run(() => repo().updatePreferences(userId, payload)),

    // ---- Account lifecycle ----
    scheduleDeletion: (userId, deletionAt) =>
      run(() => repo().scheduleDeletion(userId, deletionAt)),
    restoreProfile: (userId) => run(() => repo().restoreProfile(userId)),
    permanentlyDeleteProfile: (userId) =>
      run(() => repo().permanentlyDeleteProfile(userId))
  };
}
