...
function checkVapidConfig(): {
  ...
  // URL, but admins often set just a bare email (e.g. "admin@example.com").
  // Without normalization, setVapidDetails() throws
  ...
  try {
    ...
    // Distinguish between subject errors and key errors so the
    // troubleshooting steps are actually useful.
    const isSubjectError = errMsg
      .toLowerCase()
      .includes("subject is not a valid url");
    return {
      publicKey: pubReport,
      privateKey: privReport,
      configured: false,
      error: isSubjectError
        ? `VAPID_CONTACT_EMAIL is not a valid URL. Raw value: "${rawContact}". ` +
          `web-push requires either a mailto: URL (e.g. "mailto:admin@example.com") ` +
          `or an https URL. The server-side code now auto-prepends "mailto:" to ` +
          `bare emails, so this error should not occur after the next deploy. ` +
          `Original error: ${errMsg}`
        : `webPush.setVapidDetails() rejected the keys: ${errMsg}. ` +
