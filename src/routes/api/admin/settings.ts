...
const DEFAULTS: AllSettings = {
  ...
  rate_limits: { api_per_min: 60, auth_attempts_per_hr: 20, upload_mb_per_day: 50 },
  tmdb_settings: { cache_ttl_days: 30, fallback_language: "en", include_adult: false },
...
function validateSiteSettings(input: unknown): SiteSettings {
  if (typeof input !== "object" || input === null) throw new Error("must be an object");
  const obj = input as Record<string, unknown>;
  const defaults = DEFAULTS.site_settings;
  const social = (obj.social_links && typeof obj.social_links === "object"
    ? obj.social_links
    : {}) as Record<string, unknown>;
  return {
    site_name: typeof obj.site_name === "string" ? obj.site_name.slice(0, 60) : defaults.site_name,
    tagline: typeof obj.tagline === "string" ? obj.tagline.slice(0, 120) : defaults.tagline,
    contact_email:
      typeof obj.contact_email === "string" ? obj.contact_email.slice(0, 120) : defaults.contact_email,
    support_url: typeof obj.support_url === "string" ? obj.support_url.slice(0, 500) : "",
    privacy_url: typeof obj.privacy_url === "string" ? obj.privacy_url.slice(0, 500) : "",
    terms_url: typeof obj.terms_url === "string" ? obj.terms_url.slice(0, 500) : "",
    social_links: {
      twitter: typeof social.twitter === "string" ? social.twitter.slice(0, 200) : "",
      instagram: typeof social.instagram === "string" ? social.instagram.slice(0, 200) : "",
      github: typeof social.github === "string" ? social.github.slice(0, 200) : "",
    },
  };
}
  ...
  return {
    cache_ttl_days:
      typeof obj.cache_ttl_days === "number"
        ...
        : 30,
    fallback_language:
      typeof obj.fallback_language === "string" && obj.fallback_language.length <= 10
        ? obj.fallback_language
        : "en",
    include_adult: typeof obj.include_adult === "boolean" ? obj.include_adult : false,
  };
}
...
function validateRetentionPolicy(input: unknown): RetentionPolicy {
  ...
  return {
    soft_deleted_profiles_days: clamp(obj.soft_deleted_profiles_days, 1, 3650, 90),
...
export async function GET(event: APIEvent) {
  ...
  try {
    ...
    // Merge with defaults
    const result: Record<string, { value: unknown; updated_at: string | null }> = {};
