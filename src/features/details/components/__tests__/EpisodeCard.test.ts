...
import { canOpenEpisodeFeedback, canRateEpisode } from "../EpisodeCard";
...
describe("canRateEpisode", () => {
  ...
  it("opens feedback for a watched episode even when numeric ratings are hidden", () => {
    expect(
      canOpenEpisodeFeedback({
        inVault: true,
        isWatched: true,
        isCurrent: false,
        onFeedback: vi.fn()
  ...
  it("does not open feedback for an unwatched future episode", () => {
    expect(
      canOpenEpisodeFeedback({
        inVault: true,
        isWatched: false,
