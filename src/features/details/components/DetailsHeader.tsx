...
export default function DetailsHeader(props: DetailsHeaderProps) {
  ...
  return (
    <div class="relative">
      ...
      {/* Poster + title row */}
      <div class="px-6 md:px-8 -mt-20 md:-mt-24 relative z-10 flex items-end gap-4 mb-3">
        {/* Poster */}
        <div class="shrink-0 hidden sm:block" style="width: 96px">
          <Show
            when={posterUrl()}
            fallback={
              <div
                class="w-24 h-36 rounded-xl border flex items-center justify-center"
                style={{
                  background: "#141414",
                ...
                aria-hidden="true"
              >
                <Icon name="movie" style="color: var(--dim); font-size: 24px" />
              </div>
            }
          >
            <img
              src={posterUrl()}
              alt=""
              class="w-24 h-36 rounded-xl object-cover"
              style={{
        ...
        <div class="flex-1 min-w-0 flex justify-between items-start gap-2">
          <div class="min-w-0 flex-1">
            ...
            <div class="flex items-center gap-2 mt-2 flex-wrap">
              <span class="type-subtitle">{year()}</span>
              <Show when={year()}>
                <span style="color: var(--dim)">·</span>
              </Show>
              <span class="type-subtitle">{isTv() ? "SERIES" : "MOVIE"}</span>
              <Show when={runtime()}>
                <span style="color: var(--dim)">·</span>
                <span class="type-subtitle">{runtime()}m</span>
              </Show>
          ...
          <Show when={props.baseItem}>
            <button
              ...
              class="w-10 h-10 rounded-full flex items-center justify-center active:scale-95 transition-all shrink-0"
              style={
                props.isEditing
