...
const SyncCadenceCard: Component = () => {
  ...
  const cadenceDesc = createMemo(() => {
    const c = syncCadence();
    if (c === "realtime")
      return "Push every change to the cloud instantly. Best when you have stable internet.";
    if (c === "wifi-only")
      return "Queue changes locally and sync only when on Wi-Fi. Saves mobile data.";
    return "Only sync when you tap 'Sync now' below. Best for low-data plans or offline use.";
  });

  return (
    <div class="setting-group">
      <ControlRow icon="cloud_sync" label="Sync cadence" desc={cadenceDesc()}>
        <Segmented
          options={OPTIONS}
          current={syncCadence}
          onChange={(id) => {
            setSyncCadence(id);
            showToast(
              `Sync set to ${id === "realtime" ? "real-time" : id === "wifi-only" ? "Wi-Fi only" : "manual"}`,
              "info",
              1500
            );
          }}
      ...
      <Show when={!shouldSyncNow()}>
        <div
          class="info-callout"
          style={{ margin: "var(--sp-2) var(--sp-5) var(--sp-3)" }}
        >
          <span
          ...
          <p class="info-callout-body">
            <strong>Syncing is paused.</strong> Your changes are saved locally
            and will sync when your cadence allows.
          </p>
        </div>
      </Show>
    </div>
