...
export default function FranchiseGrid() {
  ...
  const { curatedCollections, getCollectionProgress, addUniverseToPrefs } =
    useCollections();

  const [expandedFranchise, setExpandedFranchise] = createSignal<string | null>(
    null
  );
  ...
  const franchiseProgress = (franchise: Franchise) => {
    ...
    const pct =
      totalEntries > 0 ? Math.round((totalOwned / totalEntries) * 100) : 0;
                ...
                <div class="franchise-card-left">
                  ...
                  <div class="franchise-card-info">
                    ...
                    <p class="franchise-card-meta">
                      {franchise.universes.length} universe
                      {franchise.universes.length !== 1 ? "s" : ""}
                  ...
                  <For each={franchise.universes}>
                    {(uni) => {
                      const col = createMemo(() =>
                        findCollection(uni.collectionId)
                      );
                      const uniProgress = createMemo(() =>
                        col()
                          ? getCollectionProgress(col()!, watchlist())
                          : { owned: 0, total: 0, pct: 0, missing: 0 }
                      );
                            ...
                            fallback={
                              <Show when={uni.collectionId}>
                                <button
                                  ...
                                  onClick={() =>
                                    addUniverseToPrefs(uni.collectionId!)
                                  }
                          ...
                          >
                            ...
                            >
                              ...
                              <div class="franchise-universe-chip-info">
                                <span class="franchise-universe-chip-name">
                                  {uni.name}
                                </span>
