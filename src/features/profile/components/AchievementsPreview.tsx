...
const BADGES: BadgeDef[] = [
  ...
  },
  {
    ...
    icon: "video_library",
    progress: (l) => ({
      unlocked: l.length >= 10,
      current: Math.min(l.length, 10),
      target: 10
    })
  },
  {
    ...
    icon: "movie_filter",
    progress: (l) => ({
      unlocked: l.length >= 50,
      current: Math.min(l.length, 50),
      target: 50
    })
  },
  {
    ...
    icon: "auto_awesome",
    progress: (l) => ({
      unlocked: l.length >= 100,
      current: Math.min(l.length, 100),
      target: 100
    })
  },
  {
