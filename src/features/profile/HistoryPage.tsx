...
const HistoryPage: Component = () => {
  ...
  const [statusFilter, setStatusFilter] = createSignal<"all" | "completed" | "watching" | "planned">("all");
  ...
  const grouped = createMemo<HistoryGroup[]>(() => {
    ...
    if (sf !== "all") {
      filtered = filtered.filter((m) => {
        if (sf === "planned") return m.status === "Planned" || m.status === "Plan to Watch";
    ...
    <PageContainer width="narrow" paddingTop="0" paddingBottom="var(--sp-12)">
      <div class="sec-page sec-fade-in">
        ...
        <div class="sec-body" style={{ "margin-bottom": "var(--sp-4)" }}>
          ...
          {/* Status filter tabs */}
          <div class="quick-filter-bar" style={{ "margin-top": "var(--sp-3)" }} role="tablist" aria-label="Filter history by status">
            <For each={[
              { v: "all", label: "All" },
              ...
              {(tab) => (
                <button
                  ...
                  onClick={() => setStatusFilter(tab.v as "all" | "completed" | "watching" | "planned")}
                  role="tab"
                    ...
                    <div class="history-group">
                      <div class="history-group-header">
                        ...
                        <span class="history-group-count">
                          {group.items.length} {group.items.length !== 1 ? "titles" : "title"}
                        </span>
                      </div>
                          ...
                          {(item) => (
                            ...
                            >
                                ...
                                <Show
                                  ...
                                  fallback={
                                    <div class="history-poster-fallback">
                                      <span class="material-symbols-outlined" style={{ "font-size": "16px" }} aria-hidden="true">movie</span>
                                    </div>
                                  }
                                >
                                  <img
                              ...
                              <div class="history-info">
                                <p class="history-title">{item.title || item.name}</p>
                                <p class="history-meta">
                                  {item.media_type === "tv" ? "Series" : "Movie"}
                                  <Show when={item.release_date || item.first_air_date}>
                                    {" · "}{(item.release_date || item.first_air_date || "").split("-")[0]}
                                  </Show>
                                  <Show when={getDate(item).toLocaleDateString("en-US", { month: "short", day: "numeric" })}>
                                    {" · "}{getDate(item).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
                                  </Show>
                                </p>
                              </div>
                              ...
                              <span class={`history-status ${
                                item.status === "Completed" ? "history-status-completed" :
                                item.status === "Watching" ? "history-status-watching" :
                                "history-status-planned"
                              }`}>
                                {item.status === "Plan to Watch" ? "Planned" : item.status}
                              </span>
                            </div>
                          )}
                        </For>
