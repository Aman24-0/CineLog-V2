...
const SettingsPage: Component = () => {
  ...
  const accountRows: SettingRowDef[] = [
    { href: "/settings/account", label: "Account", desc: "Email, password, OAuth providers, 2FA, sessions", icon: "manage_accounts" },
  ];

  const profileRows: SettingRowDef[] = [
    { href: "/settings/profile-preferences", label: "Profile & Preferences", desc: "Display name, country, language, default vault status", icon: "person" },
  ];

  const appearanceRows: SettingRowDef[] = [
    { href: "/settings/appearance", label: "Appearance", desc: "Theme, accent, density, type, accessibility", icon: "palette" },
  ];

  const contentRows: SettingRowDef[] = [
    { href: "/settings/content-discover", label: "Content & Discover", desc: "Adult filter, rating cap, streaming providers, default tab, rating scale", icon: "tune" },
  ];

  const notificationsRows: SettingRowDef[] = [
    { href: "/settings/notifications", label: "Notifications", desc: "Per-category, quiet hours, weekly digest, reminder lead time", icon: "notifications" },
  ];

  const calendarRows: SettingRowDef[] = [
    { href: "/settings/calendar", label: "Calendar", desc: "First day of week, time format, release timezone, default view", icon: "calendar_month" },
  ];

  const dataRows: SettingRowDef[] = [
    { href: "/settings/sync", label: "Sync & Backup", desc: "Cloud status, sync cadence, import (CSV/JSON), export (CSV/JSON), devices, danger zone", icon: "sync" },
    { href: "/settings/privacy", label: "Privacy", desc: "Visibility, hide ratings in screenshots, clear search history, clear cache", icon: "lock" },
  ];

  const aboutRows: SettingRowDef[] = [
    { href: "/settings/about", label: "About & Help", desc: "Version, changelog, terms, privacy, licenses, contact, FAQ, developer tools", icon: "info" },
  ];

  const sessionRows: SettingRowDef[] = [
    { label: "Sign Out", desc: "End your session on this device", icon: "logout", danger: true, onClick: handleSignOutClick },
  ];
      ...
      <div class="sec-page sec-fade-in">
        ...
        <div class="sec-header">
          ...
          <p class="sec-subtitle">
            Account, preferences, content, sync, privacy, and help — all in one place.
          </p>
        </div>
        ...
        <div class="sec-body">
          ...
          <section class="sec-section">
            ...
            <div class="setting-group">
              {notificationsRows.map(renderRow)}
            </div>
          </section>
            ...
            >
              <div style={{ display: "flex", "align-items": "center", gap: "var(--sp-3)", "margin-bottom": "var(--sp-3)" }}>
                <div style={{
                  ...
                  <span class="material-symbols-outlined" style={{ "font-size": "20px" }} aria-hidden="true">logout</span>
                </div>
                <h3 style={{ margin: "0", "font-family": "'Outfit', sans-serif", "font-size": "1.0625rem", "font-weight": 700, color: "var(--text-strong)" }}>
                  Sign out of CineLog?
                </h3>
              </div>
              <p style={{ margin: "0 0 var(--sp-4) 0", "font-size": "0.875rem", color: "var(--text-body)", "line-height": 1.5 }}>
                You'll lose access to your vault on this device until you sign back in. Your data stays safe in the cloud.
              </p>
              <div style={{ display: "flex", gap: "var(--sp-2)" }}>
                <button
                  ...
                  disabled={signingOut()}
                  style={{ flex: "1", background: "#ff2d55", "box-shadow": "0 0 0 1px rgba(255,45,85,0.4), 0 4px 12px rgba(255,45,85,0.2)" }}
                >
                  <Show when={!signingOut()} fallback="Signing out…">Sign Out</Show>
                </button>
                <button
                  type="button"
